...
生成阶段采用 `name-factory-v5` 候选友好窗口协议。本地程序先按分句提取距离 1–4 的自然双字机会，再以女性感、易用性、适度少见度、连续关系和来源 MMR 排序；模型只看到每段最高质量的少量窗口，并从形如 `[0]柔 [1]嘉` 的 `indexedText` 复制位置。自动候选的两个字必须来自同一个原始 passage，姓名、出现序号、取字关系、稳定 ID 和证据说明仍全部由本地程序计算。越界、跨段落、重复位置或确定虚词残片会被淘汰并写入审核报告。
