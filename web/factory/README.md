...
生成阶段采用 `name-factory-v4` 预编号指针协议：模型可以阅读批次内的古籍原文，并从形如 `[0]柔 [1]嘉` 的 `indexedText` 直接复制两个 `passageId/index` 位置，但不能自行填写姓名或出处事实。姓名用字、同字出现序号、取字关系、稳定 ID 和证据说明全部由本地程序从 `normalizedText` 计算。越界、错段落、重复位置或包含确定虚词残片的组合会被淘汰，并写入本地审核报告。
