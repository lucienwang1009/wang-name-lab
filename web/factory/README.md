...
生成与审核采用 `name-factory-v6` 协议。取材层先按分句提取距离 1–4 的自然双字机会，再以女性感、易用性、适度少见度、连续关系和来源 MMR 排序；模型只看到每段最高质量的少量窗口，并从形如 `[0]柔 [1]嘉` 的 `indexedText` 复制位置。自动候选的两个字必须来自同一个原始 passage，姓名、出现序号、取字关系、稳定 ID 和证据说明仍全部由本地程序计算。姓名感与对抗审核使用本地拼音作为读音事实，只有实质未决风险进入人工复核。越界、跨段落、重复位置或确定虚词残片会被淘汰并写入审核报告。
