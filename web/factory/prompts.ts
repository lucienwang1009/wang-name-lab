...
“王令仪”只说明整体偏好，不能因为共享“仪”或只做近义替字而加分；每个名字必须独立成立。
...
export function generationRequest(
  ...
  return {
    ...
    instructions: [
      ...
      `必须先在批次内横向比较，只留真正值得后续审核的组合；宁缺毋滥，本批通常不超过 ${Math.max(2, Math.ceil(batch.passages.length * 0.75))} 组。`,
      ...
      "不要因为原句家喻户晓就直接摘用熟悉成词、景物词或形容词；家庭明确不喜欢简单熟悉、一般人一眼就想到的名字。",
    ].join("\n\n"),
...
export function adversarialReviewRequest(
  ...
  return {
    ...
    instructions: [
      ...
      "semanticReviewRisks 与 nameReviewRisks 是前序审核已发现的具体线索；你必须独立复核，不得因前序最终 approve 就忽略其中的熟悉、像词组、谐音、多音或性别风险。",
    ...
    input: {
      finalists: proposals.map((proposal) => ({
        ...
        semanticReviewRisks: semanticById.get(proposal.proposalId)?.risks ?? [],
        ...
        nameReviewRisks: nameById.get(proposal.proposalId)?.risks ?? [],
        localPronunciation: localPronunciation(proposal),
      })),
    },
